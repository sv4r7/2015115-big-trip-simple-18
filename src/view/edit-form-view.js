...
const createEventTypeItem = (currentType) => OFFER_BY_TYPE.map( (type) => `<input id="event-type-${ type }" 
...
const createEditFormElement = (state, destinations, offers) => {
  ...
  const getCurrenPointId = (currentCityName) => {
  ...
  const getCurrenPointDescription = (currentCityName) => {
    const cityName = destinations.filter( (element) => element.name === currentCityName );
    const destinationDescription = (Object.entries(cityName).map( (descript) => descript[1].description ) );
    return destinationDescription[0] === undefined || destinationDescription[0] === null ? description : destinationDescription[0];
  };

  const getCurrenPointPhotos = (currentCityName) => {
    const cityName = destinations.filter( (element) => element.name === currentCityName );
    const destinationPhotos = (Object.entries(cityName).map( (photos) => photos[1].pictures ) );
    return currentCityName === null || currentCityName === undefined ? pictures : destinationPhotos[0];
  };
  ...
  ${ DESTINATION_POINT_NAMES.map( (point) => `<option value="${ point }" ${ point === town ? 'checked' : '' } ${ getCurrenPointId(town) }></option>` ).join('') }
  </datalist>
  </div>`);
...
<form class="event event--edit" action="#" method="post">
  <header class="event__header">
    ...
    ${ createDestinations(currentPointCityName === null ? name : currentPointCityName) } 
  ...
  <section class="event__details">
    <section class="event__section  event__section--offers">
      ...
      ${ createEventOffers(allOffers) }
      </div>
    </section>

    <section class="event__section  event__section--destination">
      <h3 class="event__section-title  event__section-title--destination">Destination</h3>
      <p class="event__destination-description">${ getCurrenPointDescription(currentPointCityName) }</p>

      <div class="event__photos-container">
        <div class="event__photos-tape">
          <img class="event__photo" src="${ getCurrenPointPhotos(currentPointCityName)[0].src }" alt="${ getCurrenPointPhotos(currentPointCityName)[0].description }">
          <img class="event__photo" src="${ getCurrenPointPhotos(currentPointCityName)[1].src }" alt="${ getCurrenPointPhotos(currentPointCityName)[1].description }">
          <img class="event__photo" src="${ getCurrenPointPhotos(currentPointCityName)[2].src }" alt="${ getCurrenPointPhotos(currentPointCityName)[2].description }">
          <img class="event__photo" src="${ getCurrenPointPhotos(currentPointCityName)[3].src }" alt="${ getCurrenPointPhotos(currentPointCityName)[3].description }">
          <img class="event__photo" src="${ getCurrenPointPhotos(currentPointCityName)[4].src }" alt="${ getCurrenPointPhotos(currentPointCityName)[4].description }">
        </div>
      </div>
    </section>
  </section>
